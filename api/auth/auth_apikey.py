...
# 文件作用
# 该文件定义了一个用于基于API密钥的身份验证类APIKeyAuth，它继承自FastAPI的APIKeyHeader类，目的是从HTTP请求头中提取API密钥并对其进行验证。通过这种方式，确保只有提供有效API密钥的用户才能访问特定API接口。
...
# __call__
# 描述:
# 该方法会在每次请求时调用，用于从请求中提取API密钥并进行验证。它首先从Authorization头中获取API密钥，然后调用自定义的authorizers.api_key_authorizer函数来检查该密钥的有效性。如果API密钥有效，它将请求上下文中的authorizer_identity设置为api_key，并将当前上下文的租户ID添加到request.state.currentContext。

# 参数:

# request (Request):
# FastAPI的请求对象，包含请求的所有信息。
# 返回值:
